...
<x-admin-layout :breadcrumbs="$breadcrumbs">
    {{-- Contenedor centralizado con Alpine.js para controlar el spinner de carga y el estado de replicación --}}
    <div class="max-w-7xl mx-auto py-8 px-4" x-data="{ loading: false, showReplicate: false, selectedDoctorId: '' }">
        
        <!-- ALERTAS DEL ECOSISTEMA GLOBAL -->
        @if (session('success'))
        ...
        <!-- DISTRIBUCIÓN ADAPTATIVA DE TRABAJO (2 COLUMNAS FORMULARIO / 3 COLUMNAS LISTADOS) -->
        <div class="grid grid-cols-1 lg:grid-cols-5 gap-6 items-start">
            <!-- COLUMNA IZQUIERDA: FORMULARIO DE ASIGNACIÓN (2 COLUMNAS) -->
            <div class="lg:col-span-2 bg-white border rounded-[2.5rem] p-6 md:p-8 shadow-sm border-slate-100">
                ...
                <form action="{{ route('partner.clinic.schedules.store') }}" method="POST" @submit="loading = true" class="space-y-4">
                    @csrf
                    ...
                    <div class="flex flex-col">
                        ...
                        <select name="address_id" required class="w-full rounded-2xl border-slate-200 py-3.5 px-4 text-sm text-slate-500 bg-white focus:ring-2 focus:ring-indigo-500 shadow-inner">
                            <option value="" disabled selected>Selecciona la ubicación física</option>
                    ...
                    <div class="flex flex-col">
                        ...
                        <select name="day" required class="w-full rounded-2xl border-slate-200 py-3.5 px-4 text-sm text-slate-500 bg-white focus:ring-2 focus:ring-indigo-500 shadow-inner">
                            ...
                            @foreach($daysMap as $num => $label) <option value="{{ $num }}">{{ $label }}</option> @endforeach
                        </select>
                    </div>
                    ...
                    <!-- Horas del Bloque: Conmutador e Interfaz para Replicar Días -->
                    <div class="pt-2">
                    ...
                    <div class="flex justify-end pt-3 border-t border-slate-50">
                        <button type="submit" :disabled="loading" class="bg-indigo-600 hover:bg-indigo-700 text-white font-black px-8 py-3.5 rounded-2xl text-xs uppercase tracking-wider shadow-lg shadow-indigo-100 transition-all flex items-center justify-center gap-2 w-full">
            ...
            <!-- COLUMNA DERECHA: TABLA Y LISTADO DE TURNOS ACTIVOS (3 COLUMNAS) -->
            <div class="lg:col-span-3 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        @forelse($schedules as $schedule)
                            ...
                            >
                                ...
                                <!-- Botón de eliminación con validación de seguridad corporativa -->
                ...
                <div class="bg-white p-6 rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-slate-50">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {{-- Laravel inyecta la variable $unavailabilities desde el index del controlador --}}
